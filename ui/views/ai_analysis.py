...
from core.ai.analyser import FlightAnalyser
from ui.components import drone_spinner

_CACHE_KEY = "ai_analysis_cache"
...
@st.cache_resource
def _get_analyser(api_key: str) -> FlightAnalyser:
    return FlightAnalyser(api_key)
...
def render_ai_analysis(metrics: Dict, file_key: str = "") -> None:
    ...
    if st.button("Analyse flight with Gemini", type="primary", use_container_width=True, disabled=button_disabled):
        with drone_spinner("Gemini is analysing your flight…"):
            try:
                result = _get_analyser(api_key).analyse(metrics, mode=mode, custom_question=custom_question)
